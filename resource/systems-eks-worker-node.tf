...
resource "aws_iam_role" "systems-eks-worker-node-role" {
    name = "systems-eks-prod-worker-node-role"
...
resource "aws_iam_policy" "systems-eks-worker-node-autoscailer-policy" {
    name = "systems-eks-prod-worker-node-autoscailer-policy"
    description = "Systems EKS Prod Worker Node AutoScailer Policy"
    policy = <<POLICY
{
      "Version": "2012-10-17",
      "Statement": [
...
resource "aws_iam_policy" "systems-eks-worker-node-albingresscontroller-policy" {
    name = "systems-eks-prod-worker-node-albingresscontroller-policy"
    description = "Systems EKS Prod Woerker Node Alb IngressController Policy"
    policy = <<POLICY
{
   "Version": "2012-10-17",
   "Statement": [
...
resource "aws_eks_node_group" "systems-eks-node-group" {
    ...
    node_group_name = "systems-eks-prod-node-group"
    ...
    labels = {
        "role" = "systems-eks-prod-node-group"
    }
    ...
    tags = {
        "Name" = "${aws_eks_cluster.systems-eks-cluster.name}-systems-eks-prod-worker-node-group"
    }
}